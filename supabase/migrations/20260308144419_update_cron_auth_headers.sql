-- 1. Update the "Business Hours" Sync to use the custom Doorbell Key
select cron.schedule(
  ...
  select net.http_post(
      ...
      headers:=jsonb_build_object(
          ...
          'x-sync-secret', (select decrypted_secret from vault.decrypted_secrets where name = 'service_role_key' limit 1)
      ),
      body:='{}'::jsonb,
...
-- 2. Update the "Audit Entries" Sync to use the custom Doorbell Key
select cron.schedule(
